...
//|                                                                  |
//+------------------------------------------------------------------+
bool LabelColorChange(const long chart_ID=0, // chart's ID
                      const string name="Label", // label name
                      const color text_color = C'0,0,0')   // label color
{
//--- reset the error value
   ResetLastError();
//--- change the label color
   if(!ObjectSetInteger(chart_ID, name, OBJPROP_COLOR, text_color))
   {
      Print(__FUNCTION__,
            ": failed to change text label color! Error code = ",GetLastError());
      return(false);
   }
   return(true);
}                        
